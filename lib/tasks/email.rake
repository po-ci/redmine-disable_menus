...
END_DESC
    ...
    task :receive_imap_pair => :environment do
      imap_options = {:host => ENV['host'],
                      :port => ENV['port'],
                      :ssl => ENV['ssl'],
                      :starttls => ENV['starttls'],
      ...
      Mailer.with_synched_deliveries do
        Redmine::IMAP.check_pair(imap_options, MailHandler.extract_options_from_env(ENV))
      end
    end

    task :receive_imap_odd => :environment do
      imap_options = {:host => ENV['host'],
                      :port => ENV['port'],
                      :ssl => ENV['ssl'],
                      :starttls => ENV['starttls'],
      ...
      Mailer.with_synched_deliveries do
        Redmine::IMAP.check_odd(imap_options, MailHandler.extract_options_from_env(ENV))
      end
    end
