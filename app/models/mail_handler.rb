...
class MailHandler < ActionMailer::Base
  ...
  def receive(email, options={})
    ...
    logger.info "MailHandler: searching user... time: #{Time.now.getutc}"
    ...
    if @user.nil?
      ...
      when 'create'
        ...
        if @user
          logger&.info "MailHandler: [#{@user.login}] account created time: #{Time.now.getutc}"
  ...
  def dispatch
    ...
    logger.info "MailHandler: Analyzing subject... time: #{Time.now.getutc}"
  ...
  # Creates a new issue
  def receive_issue
    logger.info "MailHandler: Creating issue... time: #{Time.now.getutc}"
  ...
  def receive_issue_reply(issue_id, from_journal=nil)
    logger.info "MailHandler: Updating issue... time: #{Time.now.getutc}"
